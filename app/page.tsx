    ...
    <div className="rejection" role="alertdialog" aria-modal="true">
      ...
      <div className="rejection__inner">
        ...
        <p className="rejection__fineprint">
          This decision is final-ish, dramatic, and probably deserved.
        </p>
        <div className="rejection__actions">
          <a
            className="button button--cream"
            href="https://vivalabeautywax.com"
            target="_blank"
            rel="noopener noreferrer"
          >
            Book a Wax Instead <span aria-hidden="true">↗</span>
          </a>
          <button className="button button--outline-light" onClick={onStartOver}>
            Start Over
          </button>
        </div>
      </div>
...
function BookingExperience({
  ...
  const [couponState, setCouponState] = useState<"" | "valid" | "invalid">("");
  ...
  const submitCase = (event: FormEvent) => {
    event.preventDefault();
    const cleanCase = caseText.trim().replace(/\s+/g, " ");
    if (cleanCase.length < 40) {
      setError("Vanessa needs at least 40 characters. Make the case compelling.");
      return;
    }
    setCaseText(cleanCase);
    setError("");
    setStep("email");
  };
  ...
  const couponApplied = couponState === "valid";
  const discountAmount = couponApplied && selectedSlot
    ? selectedSlot.originalAmount
    : 0;
  const finalAmount = selectedSlot
    ? selectedSlot.originalAmount - discountAmount
    : 0;

  const applyCoupon = (event: FormEvent) => {
    event.preventDefault();
    const normalized = couponCode.trim().toLowerCase();
    const valid = normalized === "bigdickalexg" || normalized === "alexguerra";
  ...
  const completeBooking = () => {
    ...
    const query = new URLSearchParams({
      ...
      originalAmount: String(selectedSlot.originalAmount),
  ...
  return (
    <div className="booking-shell" role="dialog" aria-modal="true">
      ...
      <div className="booking-panel">
        ...
        <div className="booking-progress" aria-label={`Step ${stepNumber[step]} of 9`}>
          <span style={{ width: `${(stepNumber[step] / 9) * 100}%` }} />
        </div>

        <div className="booking-panel__content">
          <p className="step-count">Step {stepNumber[step]} of 9</p>
          ...
          {step === "case" ? (
            <form className="booking-step" onSubmit={submitCase}>
              <span className="mini-label">Make your case</span>
              <h2>Why should Vanessa go out with you?</h2>
              <p>
                This is your moment. Be charming, specific, and convincing.
                Vanessa respects effort.
              </p>
              <label className="field">
                <span>Your case</span>
                <textarea
                  ...
                  maxLength={800}
                  rows={7}
                  placeholder="Vanessa should go out with me because…"
                />
              </label>
              <div className="field-meta">
                <span>Minimum 40 characters</span>
                <span>{caseText.length}/800</span>
              </div>
              {error ? <p className="form-error">{error}</p> : null}
              <button className="button button--dark button--wide" type="submit">
                Submit My Case <span aria-hidden="true">↗</span>
              </button>
            </form>
          ) : null}
          ...
          {step === "email" ? (
            <form className="booking-step" onSubmit={loadAvailability}>
              ...
              <p>Use the best address for your booking details.</p>
              <label className="field">
          ...
          {step === "availability" ? (
            <section className="booking-step booking-step--calendar">
              ...
              <p>Only the most exclusive openings make it onto this calendar.</p>
              {loading ? (
              ...
              {!loading && error ? (
                <div className="calendar-state">
                  <strong>Calendar temporarily unavailable</strong>
                  <p>{error}</p>
                  <button
          ...
          {step === "summary" && selectedSlot ? (
            <section className="booking-step">
              ...
              <dl className="summary-card">
                ...
                <div className="summary-card__case">
                  <dt>Your case</dt>
                  <dd>{caseText}</dd>
                </div>
                <div>
          ...
          {step === "payment" && selectedSlot ? (
            <section className="booking-step">
              ...
              <h2>{showZelle ? "Pay with Zelle." : "Choose payment."}</h2>
              {!showZelle ? (
                <>
                  <p>Apply a promotion code before choosing your payment method.</p>
                  <form className="promo-form" onSubmit={applyCoupon}>
                  ...
                  {couponState === "valid" ? (
                    <p className="coupon-status coupon-status--valid">
                      ✦ Promotion applied — your booking is complimentary.
                    </p>
                  ) : null}
                  {couponState === "invalid" ? (
                    <p className="form-error">That promotion code is not valid.</p>
                  ) : null}
                  <dl className="checkout-total">
                    <div><dt>Booking price</dt><dd>{money(selectedSlot.originalAmount)}</dd></div>
                    <div><dt>Promotion</dt><dd>−{money(discountAmount)}</dd></div>
                    <div><dt>Total</dt><dd>{money(finalAmount)}</dd></div>
                  </dl>
                  <div className="payment-stack">
                    <button className="payment-card" onClick={completeBooking}>
                    ...
                    <button className="payment-card" onClick={() => setShowZelle(true)}>
                      ...
                      <span>
                        <strong>Zelle</strong>
                        <small>Pay directly after confirmation</small>
                      </span>
                      <b>{money(finalAmount)}</b>
                    </button>
                  </div>
                ...
                <div className="zelle-panel">
                  <span className="zelle-panel__logo">Z</span>
                  <strong>{finalAmount === 0 ? "No payment required." : "Zelle selected."}</strong>
                  <p>Recipient details are provided separately after approval.</p>
                  <div className="zelle-panel__row">
                    <span>Recipient</span>
                    <b>{finalAmount === 0 ? "Not required" : "Provided after approval"}</b>
                  </div>
                  <div className="zelle-panel__row">
                    <span>Amount</span>
                    <b>{money(finalAmount)}</b>
                  ...
                  <button className="text-button" onClick={() => setShowZelle(false)}>
        ...
        <footer className="booking-panel__footer">
          <span className="privacy-dot" /> Applications are reviewed with
    ...
    <main>
      ...
      {checkoutCanceled ? (
        <div className="checkout-canceled" role="status">
          <span>Checkout was canceled. Nothing was reserved.</span>
      ...
      <nav className="nav shell" aria-label="Primary navigation">
        ...
        <div className="nav__actions">
          <div className="test-badge"><span /> By Application Only</div>
          <a
      ...
      <section className="meet shell section" id="meet">
        ...
        <div className="meet__copy">
          ...
          <p>
            ...
            valuable, highly requested, and now dramatically overpriced with
      ...
      <section className="process section shell">
        ...
        <ol className="process-list">
          {[
            ...
            ["03", "Select an available time", "Choose from Vanessa’s openings for the next 60 days."],
            ["04", "Complete checkout", "Apply a promotion code or choose your payment method."],
      ...
      <footer className="footer">
        <div className="shell footer__top">
          ...
          <p>
            Vanessa’s availability is limited, her standards are high, and
            applications are judged accordingly.
          </p>
        </div>
